...
defmodule AshToonEx.TransformerHelpers do
  @moduledoc false
  ...
  defp make_pick(dsl, get_fields) do
    keys =
      case Spark.Dsl.Transformer.get_option(dsl, [:toon], :pick, %{}) do
  ...
  defp make_steps(dsl) do
    for step <- Spark.Dsl.Transformer.get_entities(dsl, [:toon]),
