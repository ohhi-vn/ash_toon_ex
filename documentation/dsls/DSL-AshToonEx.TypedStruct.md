...
|------|------|---------|------|
| [`pick`](#toon-pick){: #toon-pick } | `list(atom) \| %{optional(:include) => list(atom), optional(:exclude) => list(atom)}` |  | Keys to pick from a record into a result. Accepts an explicit names list or a behaviour configuration map. |
